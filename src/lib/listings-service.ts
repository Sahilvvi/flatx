...
function filterMockListings(q: NearbyQuery, radius: number, limit: number): Listing[] {
  return MOCK_LISTINGS.map((l) => ({
    ...l,
    distance_m: (q.lat !== undefined && q.lng !== undefined) ? haversineMetres(q.lat, q.lng, l.lat, l.lng) : 0,
  }))
    .filter((l) => {
      if (q.bounds) {
        const [w, s, e, n] = q.bounds;
        return l.lng >= w && l.lng <= e && l.lat >= s && l.lat <= n;
      }
      return (l.distance_m ?? 0) <= radius;
    })
